...
REM Full witcher test in one shot: extract depth+poses from witcher.mp4 (skipping the
...
REM
REM Defaults: start 60s in (past the menu), fps 5, 300 frames (~60-120s of gameplay).
REM witcher.mp4 is native 60fps, so 3600 source frames = 60s.
REM   Arg 1: start_frame (source frames to skip). Default 3600.
...
set "VIDEO=C:\workspace\data\youtube\witcher.mp4"
set "EXPORT_DIR=C:\workspace\data\youtube\witcher_results"
...
echo.
echo Extracting witcher depth: start_frame=%START_FRAME% fps=%FPS% first_k=%FIRST_K%
echo   -^> %EXPORT_DIR%
echo.
